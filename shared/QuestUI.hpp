...
namespace QuestUI {
    ...
    class Register {
        public:
            ...
            template<class T = HMUI::ViewController*>
            static void RegisterAllModSettingsViewController(ModInfo modInfo, std::string title, bool showModInfo = true, DidActivateEvent didActivateEvent = nullptr) {
                static_assert(std::is_convertible<T, HMUI::ViewController*>());
                RegisterModSettings(modInfo, showModInfo, title, MenuLocation::AllViews, csTypeOf(T), Type::VIEW_CONTROLLER, didActivateEvent);
            }
            
            template<class T = HMUI::ViewController*>
            static void RegisterAllModSettingsViewController(ModInfo modInfo, std::string title, DidActivateEvent didActivateEvent) {
                static_assert(std::is_convertible<T, HMUI::ViewController*>());
                RegisterAllModSettingsViewController<T>(modInfo, title, true, didActivateEvent);
            }

            template<class T = HMUI::ViewController*>
            static void RegisterAllModSettingsViewController(ModInfo modInfo, DidActivateEvent didActivateEvent = nullptr) {
                static_assert(std::is_convertible<T, HMUI::ViewController*>());
                RegisterAllModSettingsViewController<T>(modInfo, modInfo.id, true, didActivateEvent);
            }
            
            template<class T>
            static void RegisterAllModSettingsFlowCoordinator(ModInfo modInfo) {
                static_assert(std::is_convertible<T, HMUI::FlowCoordinator*>());
                RegisterModSettings(modInfo, false, modInfo.id, MenuLocation::AllViews, csTypeOf(T), Type::FLOW_COORDINATOR);
            }
        
            template<class T>
            static void RegisterAllModSettingsFlowCoordinator(ModInfo modInfo, std::string title, bool showModInfo = false) {
                static_assert(std::is_convertible<T, HMUI::FlowCoordinator*>());
        ...
        private:
            static void RegisterModSettings(ModInfo modInfo, bool showModInfo, std::string title, MenuLocation location, Il2CppReflectionType* il2cpp_type, Type type, DidActivateEvent didActivateEvent = nullptr);
